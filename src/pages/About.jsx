...
export default function About() {
  return (
    <div>
      {/* Header band */}
      <section className="bg-(--color-pitch) py-16 sm:py-20">
        <div className="mx-auto max-w-7xl px-5 sm:px-8">
          <span className="text-xs font-semibold uppercase tracking-widest text-(--color-gold)">
            Since 1998
          </span>
          <h1 className="font-(family-name:--font-display) text-(--color-line) text-4xl sm:text-6xl mt-3 leading-tight">
            THE CLUB BEHIND
            <br />
            THE BADGE
          </h1>
          <p className="mt-5 max-w-xl text-(--color-line)/70 leading-relaxed">
      ...
      <section className="mx-auto max-w-4xl px-5 sm:px-8 py-16 sm:py-20 text-center">
        ...
        <p className="font-(family-name:--font-display) text-2xl sm:text-4xl text-(--color-pitch) mt-4 leading-snug">
      ...
      <section className="bg-(--color-concrete)">
        <div className="mx-auto max-w-3xl px-5 sm:px-8 py-16 sm:py-20">
          <h2 className="font-(family-name:--font-display) text-2xl sm:text-3xl text-(--color-pitch) mb-10">
            Club history
          </h2>
          <ol className="relative border-l border-(--color-pitch)/20 pl-8 space-y-10">
            {history.map((h, i) => (
              <li key={i} className="relative">
                <span className="absolute -left-[calc(2rem+1px)] top-0 flex h-7 w-7 items-center justify-center rounded-full bg-(--color-pitch) scoreboard-digits text-[10px] text-(--color-gold)">
                  {String(i + 1).padStart(2, "0")}
                </span>
                <span className="scoreboard-digits text-(--color-kit) font-semibold">{h.year}</span>
                <p className="mt-1 text-(--color-ink)/80 leading-relaxed">{h.text}</p>
              </li>
            ))}
          </ol>
        </div>
